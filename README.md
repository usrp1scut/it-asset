...
- **座位图** —— 网格画布绘制工位、**自动编号**(行优先/蛇形、分区);拖拽人员与共享设备落座,落座即**回写资产台账位置**(`location`)并记「位置变更」生命周期。
...
- **角色权限** —— IT 在矩阵里勾选各角色对每个模块的**查看 / 管理**,即时生效(后端 `require_perm` 逐请求校验);系统管理员恒为全权,用户管理/角色权限**锁定**、审批与报废流程角色**固定**,防越权与自锁。
...
![座位图](docs/screenshots/seatmap.png)
<sub>座位图 —— 网格画布绘制工位、自动编号,拖拽人员/共享设备落座,自动回写资产台账位置。</sub>

![角色权限](docs/screenshots/role-permissions.png)
<sub>角色权限 —— IT 勾选各角色对每个模块的查看 / 管理,即时生效;锁定 / 流程固定模块只读。</sub>
